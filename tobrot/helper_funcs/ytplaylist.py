...
async def yt_playlist_downg(message, i_m_sefg):
    ...
    try:
        ...
        pass
    cmd = ["youtube-dl", "-i", "-f", "bestvideo[ext=mp4]+bestaudio[ext=m4a]/mp4", "--external-downloader-args", "--geo-bypass-country IT", "-o", f"{fol_der}/%(playlist)s/%(playlist_index)s - %(title)s.%(ext)s", f"{url}"]
