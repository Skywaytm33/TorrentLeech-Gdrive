...
async def yt_playlist_downg(message, i_m_sefg):
    ...
    try:
        ...
        pass
    cmd = ["youtube-dl", "--verbose", "--geo-bypass-country", "IT", "--geo-bypass-ip-block", "2.228.225.27", "-i", "-f", "best[height=720][ext=mp4]+bestaudio[ext=m4a]/mp4", "--external-downloader-args", "-o", f"{fol_der}/%(playlist)s/%(playlist_index)s - %(title)s.%(ext)s", f"{url}"]
