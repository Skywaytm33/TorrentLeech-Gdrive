...
async def yt_playlist_downg(message, i_m_sefg):
    ...
    try:
        ...
        pass
    cmd = ["youtube-dl", "--verbose", "--geo-bypass-country", "PK", "-i", "-f", "best[height=720][ext=mp4]+bestaudio[ext=m4a]/mp4", "--external-downloader-args", "-o", f"{fol_der}/%(playlist)s/%(playlist_index)s - %(title)s.%(ext)s", f"{url}"]
